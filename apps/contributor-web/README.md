...
3. Contributors do **not** sign in — this app is appless (no auth). Only the organizer (the `apps/mobile` app) signs in, via Supabase email OTP; see that app's README for the required "Magic Link" email-template setup (the template must include `{{ .Token }}` so a 6-digit code is emailed instead of a link).
