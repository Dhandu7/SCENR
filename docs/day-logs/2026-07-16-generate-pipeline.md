...
|---|---|---|
| **HEIC photos fail Anthropic scoring** — `rank-media`'s `scorePhoto` passes the image by URL; Anthropic rejects HEIC ("file format invalid or unsupported"), the iPhone default. A HEIC-only trip → all photos dropped → `422 no_media` → can't generate. | **`/run`** live check (mocked unit tests couldn't catch it) | **Documented, not yet fixed** — see gaps below. Confirmed live: JPEG scores fine (`quality_score 45, candid_funny`); all 3 HEIC photos fail; render-worker's `sharp` decodes the *same* HEIC fine. Fix is a real follow-up (transcode HEIC→JPEG at upload, or route scoring images through a normalizer — Deno can't run `sharp`, so it's not a one-line edge-function patch). |
...
- **render-worker: fully working** — a real trip photo (a 2.2 MB **HEIC**, in fact) → render-worker → a real **1080×1080 JPEG** (7.2 KB) landed in the `renders` bucket; dimensions parsed and confirmed. This is the service that determines whether output "looks right," and it's uncovered by the synthetic-buffer unit tests.
