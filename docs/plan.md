...
- **Favourite override & pre-generation swap** (applies across formats, both the naive and slot-fill selectors): the organizer's favourited (`is_favourite`) photos take precedence over raw scores — for a single-photo Post the highest-scored favourite is featured by default; when a stronger non-favourite exists, SCENR surfaces a "swap to a stronger shot" **before** rendering rather than silently overriding the human's pick or re-rendering after the fact. This is why selection is split into a synchronous `rank-media` function (scores/caches the pool, returns the favourite-first pick + optional swap suggestion) and a `generate` function that renders the chosen `media_item_id`. MVP scopes favourites to the organizer only; per-user favourite precedence is post-MVP.
...
- **Days 4–6 — Live pool → naive generate → first real reveal**: W1 wires Media Pool to Realtime (grid, filters, Live indicator) + Generate Setup sheet. W2/W6 build the selection+render backend as a **naive fallback**: `rank-media` runs a single Haiku pass per trip photo tagging `quality_score` + `content_category` (cached on `media_items`), then returns the favourite-first pick (organizer's `is_favourite` wins, else top `quality_score`) plus an optional pre-generation swap suggestion — ignoring category/embeddings/theme-composition for now, so nothing blocks on the theme loader landing; `generate` renders the chosen photo. W3 completes `/render` (real crop, **no watermark**, caption placeholder, uploads output, flips status). W1 builds Generate Setup + Preview/Swap → Generating + Reveal (Post first, then Carousel, then Story single-image). W5 targets `theme_fingerprints` (including `composition_template`) fully populated by end of Day 6. **Checkpoint**: full walking skeleton — create trip → invite → appless upload → live pool → pick best shot (favourite-first, swap-before-render) → real rendered Post/Carousel/Story on Reveal.
